...
3b. **必须包含 \`--color-accent-gold\`**：与主强调色 \`--color-accent\` 区分开的琥珀金/香槟金属高亮色，用于大屏环形指标、警示态下的金色描边与高亮等；须与 mode、\`--color-primary\`、\`--color-warning\` 协调且对比度可读。未在 DESIGN.md 明示时，根据主色色相推导偏暖的金色 hex（禁止与 \`--color-accent\` 完全相同）。
...
9. **大屏图表域 \`--dv-chart-*\`**：折线/柱状/饼图组件会读取这些变量控制网格线、坐标轴线与刻度、图例字色、Tooltip、绘图区底色、饼图引导线、参考线等。\`--dv-chart-plot-bg\` 见 Schema **必填**；下列**其余**键可按需在 \`cssVariables\` 中追加（未追加时由应用内置默认推导）；若追加则每一项须非空，并与 mode、\`--color-text-*\`、\`--color-border\` 自洽且保证图表区对比度：
   - \`--dv-chart-plot-bg\`：**必填**；必须等价于主文本色 **15%** 与 transparent 的 \`color-mix\`（**0.15 不透明度语义**），**唯一允许取值**为 \`color-mix(in srgb, var(--color-text-primary) 15%, transparent)\`；禁止其它比例、禁止不透明实色 hex、禁止省略该键。
   ...
   - **图表/表格标题区（可选覆盖）**：\`--dv-chart-title-color-backdrop\`（矢量底纹上主标题字色，默认继承 \`--color-text-primary\`）；\`--dv-chart-title-font-size\` / \`--dv-chart-title-font-size-compact\`；\`--dv-chart-title-font-weight\` / \`--dv-chart-title-font-weight-compact\`；\`--dv-chart-title-line-height\`；\`--dv-chart-title-font-family\`（建议 \`var(--font-display)\`）；\`--dv-chart-title-gap-after-title\`；\`--dv-chart-title-subtitle-font-size\`；\`--dv-chart-title-block-margin-bottom\`；\`--dv-chart-title-backdrop-padding\`。未写出时由应用默认引用全局 \`--font-*\` / \`--space-*\`。
10. **大屏图表/表格 Widget 外层壳（必填，与 generate-jsx / token-demo 对齐）**：\`cssVariables\` 必须包含且仅允许以下取值语义——\`--dv-chart-panel-padding\` 必须为 \`0\`；\`--dv-chart-panel-border\` 必须为 \`none\`；\`--dv-chart-panel-radius\` 必须为 \`0\`；\`--dv-chart-panel-bg\` 必须为带 \`color-mix\` 的弱 \`var(--color-muted)\`（或等价）底，使图表区无卡片白底、无内边距、无圆角、无描边；\`--dv-chart-plot-bg\` 必须为第 9 条所列固定串（流水线亦会强制写入）。
11. **KPI 指标卡专属 Token（--kpi-* 五项）必须始终产出"深色卡片 + 浅色文字"的视觉组合，不论 mode 是 light 还是 dark**：
   ...
   - （可选）\`--kpi-glow-base\`：主数值外发光用的**色相**（填 \`var(--color-primary)\` 或品牌强调色即可），**禁止**填 \`var(--color-border)\` 或把「整条 text-shadow」误绑成单色；默认不发光的主题可不输出该键（应用侧默认为 transparent）。发光的多层 blur 由运行时公式生成，无需手写 text-shadow 字符串。
   - 这 5～6 个 token 的色调可以与品牌主色产生呼应（例如在深色基底上混入品牌色相），但底色始终为深色、文字始终为浅色，不得被 mode 影响。
